...
N = int(input("Number of parallel CSTRs: "))
F_total = float(input("Total liquid flow rate F_total (m^3/s): "))
...
# Each reactor gets equal volume and flow
V = V_total / N
F = F_total / N

# ===================== MODEL =====================
def parallel_cstr(t, y):
    ...
    for i in range(N):
        ...
        # CSTR balances for each parallel reactor
        dydt[idx]   = (F/V)*(C_Na2CO3_in - C_Na2CO3) - r_eff
...
sol = solve_ivp(parallel_cstr, [0, t_final], y0, t_eval=t_eval, method="BDF")
...
for s in range(4):
    ...
    plt.title(f"{species_labels[s]} in each parallel reactor")
    plt.legend()
    plt.grid()
...
plt.title("Carbon Capture Effectiveness in Each Parallel Reactor")
plt.legend()
plt.grid()
plt.show()
...
# Compute total CaCO3 from all reactors
CaCO3_total = np.zeros_like(sol.t)
for i in range(N):
    CaCO3_total += sol.y[i*4+3]

# Carbon capture effectiveness (%) for the entire system
eta_capture_total = CaCO3_total / (C_Na2CO3_in * V_total) * 100

plt.plot(sol.t, eta_capture_total, color="darkgreen", linewidth=2, label="Total Parallel System")
plt.xlabel("Time (s)")
plt.ylabel("Overall Carbon Capture (%)")
plt.title("Overall Carbon Capture Effectiveness (Parallel CSTRs)")
plt.grid()
plt.legend()
plt.show()
